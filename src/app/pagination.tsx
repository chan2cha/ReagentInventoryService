...
export function Pagination({ page, totalPages, total, pathname, paramName="page", preserve={} }: { page: number; totalPages: number; total: number; pathname: string; paramName?:string; preserve?:Record<string,string|undefined> }) {
  ...
  return <nav className="pagination" aria-label="페이지 이동"><span>전체 {total}건 · {start}-{end}</span><div>{page > 1 ? <ProgressLink href={href(page-1)}>이전</ProgressLink> : <span className="disabled">이전</span>}<strong>{page} / {totalPages}</strong>{page < totalPages ? <ProgressLink href={href(page+1)}>다음</ProgressLink> : <span className="disabled">다음</span>}</div></nav>;
}
