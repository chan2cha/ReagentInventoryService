...
import { confirmProactiveReplacement, excludeProactiveReplacement, shipProactiveReplacement } from "./actions";
...
const statusTone = { CONFIRMED: "warn", COMPLETED: "ok", EXCLUDED: "muted" } as const;
const dispositionLabel = { COLLECTED_DISPOSED: "회수 후 폐기", CLIENT_DISPOSED: "거래처 현장 폐기", NOT_COLLECTED: "미회수" } as const;
const originLabel = { EXPIRY: "유통기한", PRODUCT_DEFECT: "제품 하자" } as const;
...
export default async function ReplacementsPage() {
  ...
  return <AppShell active="/replacements" title="사후 관리" description="유통기한 임박 또는 제품 하자 납품분을 확인하고 신규 LOT로 교환합니다.">
    ...
    {user.role === "ADMIN" ? <div className="page-toolbar replacement-page-toolbar"><ReplacementPolicyDialog detectionDays={data.policy.detectionDays} minimumDeliveryShelfDays={data.policy.minimumShelfLifeDays} /><DefectReplacementDialog candidates={data.manualCandidates} /></div> : null}
